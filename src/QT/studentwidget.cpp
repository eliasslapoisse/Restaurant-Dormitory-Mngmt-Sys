...
void StudentsWidget::onAssignToRoom() {
    ...
    // Find the specific Block and Room to assign
    Block* block = dorm->getBlock(dlg.blockName().toStdString());
    if (!block) { QMessageBox::warning(this, "Error", "Block not found."); return; }

    Room* room = block->getRoom(dlg.floor(), dlg.roomNumber());
    if (!room) { QMessageBox::warning(this, "Error", "Room not found."); return; }

    if (room->addResident(*s)) {
